      ...
      >
        ...
        >
          <h2 class="text-center">
            <prismic-rich-text :field="data.data.homepage_title" />
          </h2>
        </div>
        ...
        >
          <div data-aos="fade" class="col-12 align-self-center mb-4 p-0">
            <h2 class="text-center call-to-action-title">
              Ready for empowered action and change? Book a free discovery call
              now
            </h2>
          </div>
...
<script setup>
import text_testimonial from "~/components/TestimonialQuotes.vue";
import video_testimonial from "~/components/VideoTestimonial.vue";
const { client } = usePrismic();
const { data: data } = await useAsyncData("data", () =>
  client.getSingle("testimonials")
);
